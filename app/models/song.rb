    ...
    def self.create_songs(songs, artist_id)
        songs.each do |song|
            if !Song.find_by(title: song['title'])
                ...
                Song.create(fullTitle: song['full_title'], id: (artist_id.to_s + song['id'].to_s).to_i, lyrics: lyrics, artistId: artist_id, url: song['url'], image: song["song_art_image_url"], title: song['title'])
            end
        end
    end
